...
class PerformanceProfiler:
    ...
    def __init__(self, operation_name: str, input_size_mb: float = 0.0,
...
class PerformanceMonitor:
    ...
    def get_operation_stats(self, operation_name: str) -> Dict[str, Any]:
        ...
        throughputs = [m.throughput_mbs for m in metrics_list if m.success and m.throughput_mbs > 0]
    ...
    def get_overall_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'overall_success_rate': successful_operations / total_metrics if total_metrics else 0.0,
            'operations_summary': {
                op: self.get_operation_stats(op) for op in all_operations
            },
            'system_info': self.system_info,
            'monitoring_period': {
                'start_time': min(m.start_time for m in self.metrics_history) if self.metrics_history else None,
                'end_time': max(m.end_time for m in self.metrics_history) if self.metrics_history else None
            }
        }
...
class XPCSBenchmarkSuite:
    ...
    def run_comprehensive_benchmark(self, sample_data_path: Optional[str] = None) -> BenchmarkResult:
        ...
        try:
            ...
            if sample_data_path:
                benchmark_metrics.extend(self._benchmark_io_operations(sample_data_path))
        ...
        success_rate = len(successful_metrics) / len(benchmark_metrics) if benchmark_metrics else 0.0
    ...
    def _benchmark_memory_management(self) -> List[PerformanceMetrics]:
        ...
        with PerformanceProfiler("memory_cache_operations", metadata={'test': 'cache_ops'}) as profiler:
            # Create test data
        ...
        with PerformanceProfiler("memory_pressure_detection", metadata={'test': 'pressure'}) as profiler:
            for i in range(1000):
                pressure = memory_manager.get_memory_pressure()
                cache_stats = memory_manager.get_cache_stats()

        # Test cleanup operations
        with PerformanceProfiler("memory_cleanup", metadata={'test': 'cleanup'}) as profiler:
            memory_manager._aggressive_cleanup()

        return [m for m in self.monitor.metrics_history if m.operation_name.startswith('memory_')]

    def _benchmark_io_operations(self, sample_data_path: str) -> List[PerformanceMetrics]:
        ...
        if sample_path.exists():
            ...
            with PerformanceProfiler("hdf5_enhanced_read", metadata={'file': str(sample_path)}) as profiler:
                try:
            ...
            with PerformanceProfiler("hdf5_standard_read", metadata={'file': str(sample_path)}) as profiler:
                try:
                    result = batch_read_fields(str(sample_path), ["saxs_2d"], "alias", ftype="nexus")
                    if "saxs_2d" in result and hasattr(result["saxs_2d"], 'nbytes'):
                        profiler.input_size_mb = result["saxs_2d"].nbytes / (1024 * 1024)
                except Exception as e:
    ...
    def _benchmark_g2_fitting(self) -> List[PerformanceMetrics]:
        ...
        bounds = np.array([[0.01, 1e-6, 0.8], [1.0, 1e6, 1.2]])  # [contrast, tau, baseline]
        fit_flag = np.array([True, True, True])
        fit_x = np.logspace(np.log10(np.min(tau)) - 0.5, np.log10(np.max(tau)) + 0.5, 128)

        # Benchmark sequential fitting
        with PerformanceProfiler("g2_fitting_sequential", input_size_mb=g2_synthetic.nbytes/(1024*1024),
                                metadata={'num_q': num_q, 'num_tau': num_tau}) as profiler:
            try:
                fit_line_seq, fit_val_seq = fit_with_fixed(
                    single_exp, tau, g2_synthetic, g2_err_synthetic,
                    bounds, fit_flag, fit_x
                )
            except Exception as e:
                logger.warning(f"Sequential G2 fitting test failed: {e}")

        # Benchmark parallel fitting
        with PerformanceProfiler("g2_fitting_parallel", input_size_mb=g2_synthetic.nbytes/(1024*1024),
                                metadata={'num_q': num_q, 'num_tau': num_tau}) as profiler:
            try:
                fit_line_par, fit_val_par = fit_with_fixed_parallel(
                    single_exp, tau, g2_synthetic, g2_err_synthetic,
                    bounds, fit_flag, fit_x, max_workers=4
                )
            except Exception as e:
                logger.warning(f"Parallel G2 fitting test failed: {e}")

        return [m for m in self.monitor.metrics_history if 'g2_fitting' in m.operation_name]
    ...
    def _benchmark_roi_calculations(self) -> List[PerformanceMetrics]:
        ...
        geometry_data = {
            'qmap': np.sqrt((x - center_x)**2 + (y - center_y)**2) * 0.01,  # Synthetic q-map
            'pmap': np.degrees(np.arctan2(y - center_y, x - center_x)) % 360,  # Angular map
        ...
        from ..utils.vectorized_roi import (
            PieROICalculator, RingROICalculator, ROIParameters, ROIType
        )
        ...
        with PerformanceProfiler("roi_pie_vectorized", input_size_mb=data_size_mb,
                                metadata={'roi_type': 'pie', 'shape': shape}) as profiler:
            try:
                pie_calculator = PieROICalculator()
                pie_result = pie_calculator.calculate_roi(saxs_data, geometry_data, pie_params)
            except Exception as e:
        ...
        with PerformanceProfiler("roi_ring_vectorized", input_size_mb=data_size_mb,
                                metadata={'roi_type': 'ring', 'shape': shape}) as profiler:
            try:
                ring_calculator = RingROICalculator()
                ring_result = ring_calculator.calculate_roi(saxs_data, geometry_data, ring_params)
            except Exception as e:
    ...
    def _benchmark_saxs_processing(self) -> List[PerformanceMetrics]:
        ...
        with PerformanceProfiler("saxs_log_standard", input_size_mb=data_size_mb,
                                metadata={'shape': shape}) as profiler:
            try:
                ...
                if np.sum(roi) > 0:
                    ...
                    log_result_std = np.log10(saxs_copy).astype(np.float32)
            except Exception as e:
                logger.warning(f"Standard SAXS log benchmark failed: {e}")

        # Benchmark streaming log computation
        with PerformanceProfiler("saxs_log_streaming", input_size_mb=data_size_mb,
                                metadata={'shape': shape}) as profiler:
            try:
                from ..utils.streaming_processor import process_saxs_log_streaming
                log_result_stream = process_saxs_log_streaming(saxs_data, chunk_size_mb=20.0)
            except Exception as e:
    ...
    def _create_benchmark_summary(self, metrics: List[PerformanceMetrics]) -> Dict[str, Any]:
        ...
        for op_name, op_metrics in operation_groups.items():
            ...
            if successful_metrics:
                ...
                throughputs = [m.throughput_mbs for m in successful_metrics if m.throughput_mbs > 0]
        ...
        if 'g2_fitting_sequential' in summary['performance_comparison'] and \
           'g2_fitting_parallel' in summary['performance_comparison']:
            seq_time = summary['performance_comparison']['g2_fitting_sequential']['average_duration']
            par_time = summary['performance_comparison']['g2_fitting_parallel']['average_duration']
            if par_time > 0:
...
def profile_performance(operation_name: str = None, track_input_size: bool = False):
...
@contextmanager
def benchmark_context(operation_name: str, input_size_mb: float = 0.0,
                     metadata: Optional[Dict[str, Any]] = None):
...
def run_xpcs_benchmark(sample_data_path: Optional[str] = None,
                      output_dir: str = "benchmarks") -> BenchmarkResult:
