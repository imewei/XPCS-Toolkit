...
class MathematicalInvariants:
    ...
    def correlation_function_properties():
        ...
        def test_correlation_function_positivity(intensities):
            ...
            assert np.all(g2_values >= 0), "G2 correlation function values must be non-negative"

            # Property: G2 at zero delay should be >= 1 for intensity fluctuations
            if len(g2_values) > 0:
                assert g2_values[0] >= 1.0, "G2(0) should be >= 1 for intensity fluctuations"
        ...
        def test_correlation_function_normalization(intensities):
            ...
            if len(g2_values) > 10:
                ...
                assert 0.5 <= long_time_avg <= 5.0, f"Long-time G2 average {long_time_avg} outside reasonable range"

        return [test_correlation_function_positivity, test_correlation_function_normalization]
    ...
    def fitting_properties():
        ...
        def test_exponential_fitting_convergence(x_data, amplitude, decay_rate, noise_level):
        ...
        def test_fitting_residuals_properties(data_points, signal_strength, background_level):
            ...
            fitted_params = mock_exponential_fit(x_data, y_data)
            y_fitted = fitted_params['amplitude'] * np.exp(-fitted_params['decay_rate'] * x_data)
            ...
            if not np.isnan(residual_trend):
                assert abs(residual_trend) < 0.8, f"Residual trend {residual_trend} too strong"
    ...
    def numerical_stability_properties():
        ...
        def test_matrix_operations_stability(matrix_size, condition_number):
            ...
            assert relative_error < 1e-10, f"Linear solve error {relative_error} too large"

            # Property: Matrix inverse should satisfy A * A^-1 = I
            A_inv = np.linalg.inv(A)
            identity_error = np.linalg.norm(A @ A_inv - np.eye(matrix_size))
            assert identity_error < 1e-10, f"Matrix inverse error {identity_error} too large"
        ...
        def test_floating_point_precision(array_size, scale_factor):
            ...
            relative_error = abs(sum_separate - sum_combined) / max(abs(sum_separate), 1e-16)
            assert relative_error < 1e-10, f"Floating point precision error {relative_error} too large"
    ...
    def fourier_transform_properties():
        ...
        @given(
            signal_length=st.integers(min_value=8, max_value=128)
        )
        def test_fft_parseval_theorem(signal_length):
            ...
            assert relative_error < 1e-10, f"Parseval theorem error {relative_error} too large"
...
class XPCSPropertyTests:
    ...
    def intensity_statistics_properties():
        ...
        def test_intensity_distribution_properties(pixel_count, mean_intensity, contrast):
            """Test properties of intensity distributions."""
            if not HYPOTHESIS_AVAILABLE:
                return

            # Generate synthetic intensity data
            intensities = generate_mock_intensity_distribution(pixel_count, mean_intensity, contrast)
        ...
        def test_temporal_correlation_properties(time_points, correlation_time):
            ...
            time_series = generate_mock_correlated_time_series(time_points, correlation_time)
            ...
            assert abs(autocorr_normalized[0] - 1.0) < 1e-10, "Autocorrelation at zero lag should be 1"
            ...
            if len(autocorr_normalized) > 10:
                ...
                assert late_avg < early_avg * 1.5, "Autocorrelation should show some decay pattern"

        return [test_intensity_distribution_properties, test_temporal_correlation_properties]
    ...
    def scattering_properties():
        ...
        def test_structure_factor_properties(q_values, structure_factor_amplitude):
            ...
            structure_factor = calculate_mock_structure_factor(q_values, structure_factor_amplitude)
            ...
            if len(structure_factor) > 5:
                ...
                assert high_q_ratio < 2.0, "Structure factor should not increase dramatically at high q"
...
def mock_exponential_fit(x_data: np.ndarray, y_data: np.ndarray) -> Dict[str, float]:
    ...
    try:
        ...
        mean_y = np.mean(y_data) if len(y_data) > 0 and np.isfinite(np.mean(y_data)) else 1.0
        return {
...
def generate_mock_intensity_distribution(pixel_count: int, mean_intensity: float, contrast: float) -> np.ndarray:
...
def generate_mock_correlated_time_series(time_points: int, correlation_time: float) -> np.ndarray:
    """Generate mock correlated time series."""
    # Simple AR(1) process with better decay properties
    alpha = np.exp(-1.0 / max(correlation_time, 0.1))  # Ensure reasonable correlation time
    ...
    time_series = time_series / np.std(time_series) if np.std(time_series) > 0 else time_series

    return time_series


def calculate_mock_structure_factor(q_values: np.ndarray, amplitude: float) -> np.ndarray:
...
class HypothesisTestRunner:
    ...
    def __init__(self, max_examples: int = 100, deadline: Optional[int] = None):
    ...
    def run_mathematical_invariant_tests(self) -> Dict[str, Any]:
        ...
        with warnings.catch_warnings():
            ...
            test_groups = [
                ("correlation_functions", math_invariants.correlation_function_properties()),
                ("fitting_algorithms", math_invariants.fitting_properties()),
                ("numerical_stability", math_invariants.numerical_stability_properties()),
            ...
            for group_name, test_functions in test_groups:
                ...
                for test_func in test_functions:
                    try:
                        ...
                        group_results[test_func.__name__] = f"failed: {str(e)}"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--deadline', type=int, default=None,
                       help='Test deadline in milliseconds')
    parser.add_argument('--math-only', action='store_true',
                       help='Run only mathematical invariant tests')
    parser.add_argument('--xpcs-only', action='store_true',
                       help='Run only XPCS-specific tests')
