...
class DocumentationValidator:
    ...
    def check_docstring_quality(self, module_name: str, obj_name: str = None) -> Dict[str, bool]:
...
class TestModuleDocumentation:
    ...
    def test_module_docstrings_exist(self, doc_validator):
        ...
        for module_name in doc_validator.qt_modules:
            ...
            assert quality['has_description'], f"Module {module_name} docstring lacks description"
    ...
    def test_public_functions_documented(self, doc_validator):
        ...
        for module_name in doc_validator.qt_modules:
            ...
            for func_name in functions:
                ...
                assert quality['exists'], f"Function {module_name}.{func_name} missing docstring"
    ...
    def test_thread_manager_api_coverage(self, doc_validator):
        ...
        for method in key_methods:
            if method in methods:
                quality = doc_validator.check_docstring_quality(module, 'QtCompliantThreadManager')
                assert quality['exists'], f"QtCompliantThreadManager.{method} missing documentation"
            ...
            if 'TASK_' in report_path:
                ...
                if full_path.exists():
                    ...
                    for section in required_sections:
                        assert section in content, f"Report {report_path} missing section: {section}"
    ...
    def test_technical_details_coverage(self, doc_validator):
        ...
        if task_1_path.exists():
            ...
            for concept in key_concepts:
                assert concept.lower() in content.lower(), f"Task 1 report missing concept: {concept}"
...
class TestCodeExampleValidation:
    ...
    def test_code_examples_syntax(self):
        ...
        doc_files = list(project_root.glob('*.md')) + list(project_root.glob('docs/*.md'))
        ...
        for doc_file in doc_files:
            ...
            for i, code_block in enumerate(code_blocks):
                ...
                try:
                    ...
                    if "invalid syntax" in error_msg and any("@pytest.mark" in line for line in lines):
                        continue
                    
                    # Skip incomplete code snippets with decorators/imports
                    if len(lines) <= 10 and any(line.strip().startswith(("@", "import ", "from ")) for line in lines):
                        continue
...
class TestDocumentationCompleteness:
    ...
    def test_architecture_overview_exists(self):
        ...
        for arch_file in arch_files:
            ...
            if full_path.exists():
                content = full_path.read_text()
                assert len(content) > 1000, f"Architecture documentation too brief: {arch_file}"

                # Check for architectural concepts
                arch_concepts = ['component', 'integration', 'system', 'framework']
                concept_count = sum(1 for concept in arch_concepts if concept in content.lower())
                assert concept_count >= 2, f"Architecture doc lacks architectural concepts: {arch_file}"
    ...
    def test_usage_examples_exist(self):
        ...
        for test_file in test_files:
            if 'qt' in test_file.name.lower():
                content = test_file.read_text()
                if 'from xpcs_toolkit' in content and len(content) > 500:
                    usage_examples_found = True
                    break

        assert usage_examples_found, "No comprehensive usage examples found in test files"

    def test_troubleshooting_info_exists(self):
        """Test that troubleshooting information exists."""
        completion_reports = list(project_root.glob('TASK_*_COMPLETION_REPORT.md'))

        troubleshooting_found = False
        for report in completion_reports:
            content = report.read_text()
            troubleshooting_indicators = [
                'error', 'issue', 'problem', 'fix', 'solution', 'troubleshoot'
            ]

            indicator_count = sum(1 for indicator in troubleshooting_indicators
        ...
        assert troubleshooting_found, "Insufficient troubleshooting information in documentation"
