...
class CoverageManager:
    ...
    def __init__(self, project_root: Optional[Path] = None):
        ...
        self.coverage_targets = [
            ...
            CoverageTarget("xpcs_toolkit/module/g2mod.py", 95.0, True, "G2 correlation analysis"),
            CoverageTarget("xpcs_toolkit/module/twotime.py", 95.0, True, "Two-time correlation"),
            CoverageTarget("xpcs_toolkit/module/saxs1d.py", 95.0, True, "SAXS 1D analysis"),
            CoverageTarget("xpcs_toolkit/module/saxs2d.py", 95.0, True, "SAXS 2D analysis"),
            CoverageTarget("xpcs_toolkit/helper/fitting.py", 95.0, True, "Scientific fitting algorithms"),
            CoverageTarget("xpcs_toolkit/fileIO/qmap_utils.py", 95.0, True, "Q-map calculations"),

            # Core modules (85% minimum)
            CoverageTarget("xpcs_toolkit/xpcs_file.py", 85.0, True, "Core data container"),
            CoverageTarget("xpcs_toolkit/fileIO/hdf_reader.py", 85.0, True, "HDF5 I/O operations"),
            CoverageTarget("xpcs_toolkit/viewer_kernel.py", 85.0, True, "Application kernel"),
            CoverageTarget("xpcs_toolkit/threading/", 85.0, False, "Threading subsystem"),
            CoverageTarget("xpcs_toolkit/utils/", 85.0, False, "Utility modules"),

            # Supporting modules (75% minimum)
            CoverageTarget("xpcs_toolkit/module/average_toolbox.py", 75.0, False, "Data averaging"),
            CoverageTarget("xpcs_toolkit/module/stability.py", 75.0, False, "Stability analysis"),
            CoverageTarget("xpcs_toolkit/plothandler/", 75.0, False, "Plotting subsystem"),
            CoverageTarget("xpcs_toolkit/file_locator.py", 75.0, False, "File management"),

            # GUI modules (50% minimum - hard to test)
            CoverageTarget("xpcs_toolkit/xpcs_viewer.py", 50.0, False, "Main GUI application"),
    ...
    def run_coverage_analysis(self, test_pattern: str = "tests/",
                            test_profile: str = "full") -> CoverageReport:
            ...
            for class_elem in package.findall('classes/class'):
                ...
                if filename.startswith('xpcs_toolkit/'):
                    ...
                    if lines is not None:
                        ...
                        missing = len([line for line in lines.findall('line')
                                     if line.attrib.get('hits', '0') == '0'])

                        # Branch coverage calculation
                        branches = len([line for line in lines.findall('line')
                                      if 'branch' in line.attrib])
                        partial_branches = len([line for line in lines.findall('line')
                                              if line.attrib.get('branch', 'false') == 'partial'])

                        if statements > 0:
                            coverage_pct = ((statements - missing) / statements) * 100
                            branch_pct = ((branches - partial_branches) / branches * 100) if branches > 0 else 100
    ...
    def _analyze_targets(self, report: CoverageReport):
        ...
        for target in self.coverage_targets:
            ...
            for metrics in report.module_metrics:
                if (target.module_pattern.endswith('/') and
                    metrics.module_name.startswith(target.module_pattern)) or \
                   (metrics.module_name == target.module_pattern):
                    matching_modules.append(metrics)
                    metrics._target = target.target_percentage

            if matching_modules:
                # Calculate average coverage for pattern
                avg_coverage = sum(m.coverage_percentage for m in matching_modules) / len(matching_modules)
    ...
    def _detect_regressions(self, report: CoverageReport, test_profile: str):
        ...
        with sqlite3.connect(self.coverage_db) as conn:
            ...
            for row in cursor.fetchall():
                module_name, coverage, timestamp = row
            ...
            for metrics in report.module_metrics:
                if metrics.module_name in recent_data:
                    ...
                    if previous_coverage - metrics.coverage_percentage > regression_threshold:
    ...
    def generate_detailed_report(self, report: CoverageReport) -> str:
        ...
        sorted_modules = sorted(report.module_metrics,
                              key=lambda x: x.coverage_percentage)[:10]

        for metrics in sorted_modules:
            lines.append(f"{metrics.module_name:50} {metrics.coverage_percentage:6.1f}%")
        ...
        critical_modules = [m for m in report.module_metrics
                          if any(t.critical and (
                              t.module_pattern == m.module_name or
                              (t.module_pattern.endswith('/') and m.module_name.startswith(t.module_pattern))
                          ) for t in self.coverage_targets)]

        for metrics in critical_modules:
            status = "✅" if hasattr(metrics, '_target') and metrics.coverage_percentage >= metrics._target else "❌"
            lines.append(f"{status} {metrics.module_name:40} {metrics.coverage_percentage:6.1f}%")
    ...
    def generate_json_report(self, report: CoverageReport) -> str:
        ...
        report_data = {
            ...
            'targets': {
                ...
                'success_rate': (len(report.targets_met) / len(self.coverage_targets) * 100) if self.coverage_targets else 0
            },
            'regression_alerts': {
        ...
        for metrics in report.module_metrics:
            ...
            for target in self.coverage_targets:
                if (target.module_pattern == metrics.module_name or
                    (target.module_pattern.endswith('/') and metrics.module_name.startswith(target.module_pattern))):
                    matching_target = {
                        'target_percentage': target.target_percentage,
                        'critical': target.critical,
                        'description': target.description,
                        'meets_target': metrics.coverage_percentage >= target.target_percentage
                    }
                    break
...
<body>
    ...
    {trend_chart}
        ...
        regression_alerts_section = ""
        if report.regression_alerts:
            alerts_html = "".join([f'<div class="alert">{alert}</div>' for alert in report.regression_alerts])
        ...
        targets_met_section = ""
        if report.targets_met:
            met_items = "".join([f'<li class="target-met">✅ {target}</li>' for target in report.targets_met])
            targets_met_section = f'<h3>Targets Met</h3><ul>{met_items}</ul>'

        targets_missed_section = ""
        if report.targets_missed:
            missed_items = "".join([f'<li class="target-missed">❌ {target}</li>' for target in report.targets_missed])
            targets_missed_section = f'<h3>Targets Missed</h3><ul>{missed_items}</ul>'

        # Build module table rows
        module_rows = []
        for metrics in sorted(report.module_metrics, key=lambda x: x.coverage_percentage):
            # Find matching target
            target_info = "N/A"
            row_class = ""
            status = ""
        ...
        return html_template.format(
            ...
            branch_coverage=report.branch_coverage,
            lines_covered=report.total_statements - report.total_missing,
            targets_success_rate=(len(report.targets_met) / len(self.coverage_targets) * 100) if self.coverage_targets else 0,
    ...
    def _generate_trend_chart_html(self) -> str:
        ...
        try:
            ...
            timestamps = [datetime.fromisoformat(ts.replace('Z', '+00:00')) for ts in trends['timestamps']]

            # Coverage trends
            ax1.plot(timestamps, trends['overall_coverage'], label='Line Coverage', marker='o')
            ax1.plot(timestamps, trends['branch_coverage'], label='Branch Coverage', marker='s')
            ...
            # Target success rate
            ax2.plot(timestamps, trends['target_success_rate'], label='Target Success Rate', marker='^', color='green')
    ...
    def export_coverage_trends(self, days: int = 30) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.coverage_db) as conn:
            ...
            for row in cursor.fetchall():
                ...
                trend_data['target_success_rate'].append((met / total * 100) if total > 0 else 0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--test-pattern', type=str, default='tests/',
    ...
    parser.add_argument('--format', type=str, choices=['text', 'json', 'html', 'xml'],
                       default='text', help='Report output format')
    parser.add_argument('--output', type=str, metavar='FILE',
                       help='Output file for reports (default: stdout)')
    parser.add_argument('--export-trends', action='store_true',
                       help='Export trend data in JSON format')
    parser.add_argument('--generate-all-reports', action='store_true',
