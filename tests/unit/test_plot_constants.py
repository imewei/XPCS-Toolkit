...
class TestPlotConstants(unittest.TestCase):
    ...
    def test_rgb_color_format(self):
        ...
        for i, color in enumerate(MATPLOTLIB_COLORS_RGB):
            self.assertIsInstance(color, (list, tuple), f"RGB color {i} should be list/tuple")
            ...
            for j, component in enumerate(color):
                ...
                self.assertGreaterEqual(
                    component, 0,
                    f"RGB color {i} component {j} should be non-negative"
                )
                self.assertLessEqual(
                    component, 255,
                    f"RGB color {i} component {j} should be <= 255"
                )
    ...
    def test_marker_format(self):
        ...
        for i, marker in enumerate(EXTENDED_MARKERS):
            ...
            self.assertGreater(
                len(marker), 0,
                f"Marker {i} should not be empty string"
            )
    ...
    def test_get_color_marker_function(self):
        ...
        color1, marker1 = get_color_marker(0, 'matplotlib')
        color2, marker2 = get_color_marker(len(MATPLOTLIB_COLORS_HEX), 'matplotlib')
    ...
    def test_scientific_plot_compatibility(self):
        ...
        self.assertGreaterEqual(
            len(MATPLOTLIB_COLORS_HEX), 10,
            "Should have at least 10 colors for scientific plots"
        )
        self.assertGreaterEqual(
            len(EXTENDED_MARKERS), 10,
        ...
        self.assertEqual(
            len(unique_hex_colors), len(MATPLOTLIB_COLORS_HEX),
            "All hex colors should be unique"
        )

        unique_markers = set(EXTENDED_MARKERS)
        self.assertEqual(
            len(unique_markers), len(EXTENDED_MARKERS),
