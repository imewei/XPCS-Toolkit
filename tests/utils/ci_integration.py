...
class CIEnvironmentDetector:
    ...
    def detect_ci_environment() -> Dict[str, Any]:
        ...
        elif any(os.getenv(var) for var in ['CI', 'CONTINUOUS_INTEGRATION', 'BUILD_NUMBER']):
            env_info['is_ci'] = True
...
class TestReportGenerator:
    ...
    def generate_junit_xml(self, test_suite: TestSuite, filename: str = "test-results.xml") -> Path:
        ...
        testsuite.set('timestamp', time.strftime('%Y-%m-%dT%H:%M:%S', time.gmtime(test_suite.timestamp)))

        # Add individual test cases
        for result in test_suite.results:
            testcase = ET.SubElement(testsuite, 'testcase')
            testcase.set('classname', result.test_id.split('::')[0] if '::' in result.test_id else 'unknown')
    ...
    def generate_json_report(self, test_suite: TestSuite, filename: str = "test-results.json") -> Path:
    ...
    def generate_markdown_summary(self, test_suite: TestSuite, filename: str = "test-summary.md") -> Path:
        ...
        # Status badge
        status_emoji = "✅" if test_suite.failed == 0 and test_suite.errors == 0 else "❌"
        content.append(f"**Status:** {status_emoji} {'PASSED' if test_suite.failed == 0 and test_suite.errors == 0 else 'FAILED'}")
        content.append("")

        # Failed tests section
        failed_tests = [r for r in test_suite.results if r.status in ('failed', 'error')]
        if failed_tests:
        ...
        if slow_tests:
            ...
            for result in sorted(slow_tests, key=lambda x: x.duration, reverse=True)[:5]:
        ...
        for result in test_suite.results:
            for category in result.categories:
                if category not in categories:
                    categories[category] = {'passed': 0, 'failed': 0, 'skipped': 0, 'errors': 0}
        ...
        if categories:
            ...
            for category, stats in sorted(categories.items()):
                content.append(f"| {category} | {stats['passed']} | {stats['failed']} | {stats['skipped']} | {stats['errors']} |")
            content.append("")
    ...
    def generate_coverage_badge_data(self, coverage_percentage: float, filename: str = "coverage-badge.json") -> Path:
...
class ArtifactManager:
    ...
    def collect_test_outputs(self, output_pattern: str = "test_*") -> List[Path]:
        ...
        for output_file in output_files:
            if output_file.is_file() and output_file.suffix in ['.xml', '.json', '.html', '.txt']:
    ...
    def create_archive(self, archive_name: str = "test-artifacts.tar.gz") -> Optional[Path]:
