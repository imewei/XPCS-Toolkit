...
def validate_single_dataset(generator, scenario, bounds, verbose=False):
    ...
    try:
        ...
        if verbose and not success:
            ...
            print(f"  Errors: tau={rel_err_tau:.3f}, bkg={rel_err_bkg:.3f}, cts={rel_err_cts:.3f}")
...
def compare_with_standard_optimizer(generator, scenarios, bounds, n_samples=10):
    ...
    for scenario in scenarios[:5]:  # Test on subset for comparison
        for _ in range(n_samples):
            try:
                ...
                start_time = time.time()
                try:
                    popt_robust, _, info_robust = optimizer.robust_curve_fit(
                        single_exp, tau, g2_single_exp, bounds=bounds, sigma=g2_err
                    )
                ...
                try:
                    ...
                    popt_std, _ = curve_fit(
                        single_exp, tau, g2_single_exp, p0=p0, bounds=bounds, sigma=g2_err
                    )
                    ...
                    r_squared = 1 - np.sum(residuals**2) / np.sum((g2_single_exp - np.mean(g2_single_exp))**2)
    ...
    print(f"  RobustOptimizer:  {robust_successes}/{total_tests} = {robust_successes/total_tests*100:.1f}% success")
    print(f"  Standard curve_fit: {standard_successes}/{total_tests} = {standard_successes/total_tests*100:.1f}% success")
...
"""
        ...
        for scenario, count in sorted(failure_scenarios.items(), key=lambda x: x[1], reverse=True):
...
def plot_validation_results(results, save_path=None):
    ...
    try:
        ...
        fit_times = [r['fit_time'] * 1000 for r in results if r['success']]  # Convert to ms
        if fit_times:
            ax3.hist(fit_times, bins=30, color='lightgreen', edgecolor='darkgreen', alpha=0.7)
            ax3.axvline(np.mean(fit_times), color='red', linestyle='--',
        ...
        r_squared_values = [r['r_squared'] for r in results if r['success']]
        if r_squared_values:
            ax4.hist(r_squared_values, bins=30, color='orange', edgecolor='darkorange', alpha=0.7)
            ax4.axvline(np.mean(r_squared_values), color='red', linestyle='--',
...
def main():
    ...
    parser.add_argument('--verbose', action='store_true',
                       help='Print verbose output including failures')
    parser.add_argument('--compare', action='store_true',
                       help='Compare with standard curve_fit')
    parser.add_argument('--plot', type=str, default=None,
                       help='Save validation plots to file')
    ...
    for i, scenario in enumerate(scenarios):
        ...
        for j in range(args.n_datasets):
            ...
            if args.verbose and (j + 1) % 10 == 0:
                ...
                print(f"  Progress: {j+1}/{args.n_datasets}, Success: {successes}/{j+1}")
        ...
        print(f"Scenario {scenario['name']}: {scenario_successes}/{len(scenario_results)} = {scenario_rate*100:.1f}%")
